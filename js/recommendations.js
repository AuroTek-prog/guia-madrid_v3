// js/recommendations.js - Versión FINAL con Premium destacado + Básicos por zona + filtros + logs + fallback
...
async function renderFilteredContent() {
    ...
    if (recs.sections) {
        recs.sections.forEach(section => {
            ...
            itemsContainer.className = section.titleKey === 'essentials_title'
                ? 'flex gap-4 overflow-x-auto hide-scrollbar snap-x pb-4'
                : 'flex flex-col gap-4';
    ...
    // 2. PARTNERS — Premium arriba + Básicos por zona
    try {
        console.log('Cargando partners...');
        const zone = await getApartmentZone(apt);
        console.log('Zona detectada:', zone?.name || 'No detectada');
        ...
        console.log('Partners cargados:', allPartners.length);

        const premiumPartners = allPartners.filter(p =>
            p.global === true &&
            p.active !== false &&
            (currentFilter === 'all' || p.categoryKey === currentFilter)
        );

        const basicPartners = zone
            ? allPartners.filter(p =>
                !p.global &&
                p.zones?.includes(zone.id) &&
                p.active !== false &&
                (currentFilter === 'all' || p.categoryKey === currentFilter)
            )
            : [];
        ...
        if (premiumPartners.length > 0 || basicPartners.length > 0) {
            hasContent = true;
            ...
            if (premiumPartners.length > 0) {
                ...
                premiumPartners.forEach(partner => {
                    console.log('Render premium:', partner.name);
                    const card = document.createElement('div');
            ...
            if (basicPartners.length > 0) {
                ...
                basicPartners.forEach(partner => {
                    console.log('Render básico:', partner.name);
                    const card = document.createElement('div');
            ...
            console.log('No hay partners visibles con el filtro actual');
        }
    } catch (err) {
        console.error('Error cargando partners o zonas:', err);
    }

    // 3. Fallback visual global
    if (!hasContent) {
